...
## Instructions to run: 
 ...
 c. Run the following command to generate required output file: ```docker run  -v <to-update-root-direcotry>/data:/opt/heart_rate_flink/data  pyflink:mysense  /etc/poetry/bin/poetry run python /opt/heart_rate_flink/src/starter_template.py```
...
Create a new branch (if given access to github repo) with the modified code (there should be one file for task A and one for task B) and provide commands such as above to run and get desired output in the output directory. 
